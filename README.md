...
1. **Convolutional Layers**: 5 layers with increasing filters (32→256)
2. **Batch Normalization**: After each convolutional layer
3. **MaxPooling Layers**: 3 layers for dimensionality reduction
4. **Fully Connected Layers**: 2 dense layers with dropout regularization
5. **Output Layer**: Softmax activation for 3-class classification
...
- **Training Details**:
  ...
  - Learning rate reduced from 0.000125 to 0.0000625 at epoch 19
  - Model weights restored from best epoch (17)
...
1. **لایه‌های کانولوشنی**: ۵ لایه با فیلترهای افزایشی (۳۲→۲۵۶)
2. **Batch Normalization**: بعد از هر لایه کانولوشنی
3. **لایه‌های MaxPooling**: ۳ لایه برای کاهش ابعاد
4. **لایه‌های Fully Connected**: ۲ لایه متراکم با регуولاریزیشن Dropout
5. **لایه خروجی**: فعال‌سازی softmax برای طبقه‌بندی ۳ کلاس
...
- **بهترین اپوک**: ۱۷
- **بهترین دقت اعتبارسنجی**: ۹۹.۵۴٪
- **دقت نهایی آموزش**: ۹۹.۶۸٪
- **جزئیات آموزش**:
  - توقف زودهنگام در اپوک ۲۰ فعال شد
  - نرخ یادگیری در اپوک ۱۹ از ۰.۰۰۰۱۲۵ به ۰.۰۰۰۰۶۲۵ کاهش یافت
  - وزن‌های مدل از بهترین اپوک (۱۷) بازیابی شدند
